...
def get_zip_stored_entry_offset(file: mtio.MTIOBase, name: str):
    ...
    if (
        len(data) == zip_eocd_size
        and data[0:4] == zip_eocd_magic
        ...
        for length in range(1, try_sz - zip_eocd_size + 1):
            ...
            if (
                l == length
                and data[-length - zip_eocd_size : -length - zip_eocd_size + 4]
                == zip_eocd_magic
            ):
    ...
    if not (cd_num == 0xFFFF or cd_sz == 0xFFFFFFFF or cd_off == 0xFFFFFFFF):
        ...
        if eocd64_locator_off < 0:
            raise ValueError(
                f'unexpected eocd64_locator_off {eocd_off} - {zip64_eocd_locator_size}'
            )
        data2 = file.read(eocd64_locator_off, zip64_eocd_locator_size)
        if data2[0:4] != zip64_eocd_locator_magic:
            raise ValueError(
                f'unexpected EOCD64Locator magic {data2[0:4]}, expected {zip64_eocd_locator_magic}'
            )
        ...
        if data2[0:4] != zip64_eocd_magic:
            raise ValueError(
                f'unexpected EOCD64Locator magic {data2[0:4]}, expected {zip64_eocd_magic}'
            )
    ...
    while i < cd_num and p < cd_sz:
        ...
        if cd[0:4] != zip_cdfh_magic:
            raise ValueError(
                f'invalid cd magic at {i=} {p=} {cd[0:4]}, expected {zip_cdfh_magic}'
            )
    ...
    if data[0:4] != zip_fh_magic:
        raise ValueError(
            f'unexpected file header magic at {lfh_offset}: {data[0:4]}, expected {zip_fh_magic}'
        )
